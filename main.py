...
@web_app.route('/')
def home():
    return "🤖 Universal Extractor Bot is Running Successfully on Render!"
...
try:
    ...
    if not os.path.exists(os.path.join(deno_path, "deno")):
        print("📥 Installing Deno for Hanime Plugin...")
    ...
    print(f"⚠️ Warning: Deno installation failed. Error: {e}")
...
if __name__ == "__main__":
    # Pehle Flask Web Server start hoga background me
    Thread(target=run_web).start()
    print("🤖 Universal Extractor Bot is Alive...")
    # Phir Pyrogram Bot chalega
    app.run()
