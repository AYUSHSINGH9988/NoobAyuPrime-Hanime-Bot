...
# 🌐 YouTube Anti-Ban Headers (Impersonating Chrome)
...
class HealthCheckHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Bot is ALIVE with YouTube Headers & Batch-Queue System!")
...
# ==========================================
# 🌟 SMART CROSS-SITE SCANNER & AGGREGATOR
# ==========================================
...
async def get_video_info_async(url):
    try:
        ...
        # 🔥 Cookies AND YouTube Headers Injected
        ...
        for f in data.get('formats', []):
            height = f.get('height')
            if height and isinstance(height, int) and f.get('vcodec') != 'none':
                qualities.add(height)
                
        return safe_title, vid_url, thumb, duration, sorted(list(qualities), reverse=True)
...
async def smart_cross_site_scan(original_url):
    ...
    for i, res in enumerate(results):
        ...
        if title:
            ...
            for q in qualities:
                if q not in aggregated_streams:
                    aggregated_streams[q] = candidates[i]
...
async def generate_screenshots(video_path, duration, num_screenshots=9):
    ...
    return screenshots
# ==========================================
# 🚦 QUEUE WORKER (Series-Level 10s Delay)
# ==========================================
...
async def queue_worker():
    while True:
        ...
        try:
            ...
            franchise_videos = q_data['episodes']
            quality = q_data['quality']
            need_ss = q_data['need_ss']
            
            await original_msg.reply_text(f"🚀 **Starting Queued Series ({len(franchise_videos)} items)...**")
            
            # Series ke andar ke episodes download kar raha hai
            for index, vid in enumerate(franchise_videos):
                if ACTIVE_TASKS.get(task_id, {}).get("cancel"):
                    await original_msg.reply_text("❌ **Queue Series stopped by user.**")
                    break
                ...
                if title:
                    ...
                    if index < len(franchise_videos) - 1: 
                        await asyncio.sleep(5) # Episodes ke beech normal 5s gap
                else:
                    await original_msg.reply_text(f"❌ **Failed for:** `{vid_url}`")
            
            if not ACTIVE_TASKS.get(task_id, {}).get("cancel"):
                await original_msg.reply_text(f"🎉 **Queued Series Complete in {quality}p!**\n⏳ *Taking a 10-second break before next task...*")

        except Exception as e:
            print(f"Queue worker error: {e}")
        finally:
            task_queue.task_done()
            # 🔥 THE MAGIC DELAY: Ek poori series khatam hone ke baad 10 second ka fix gap
...
async def process_video(client, original_message, vid_title, video_url, yt_thumb, yt_duration, quality, task_id, need_ss=False):
    ...
    # 🔥 COOKIES & YOUTUBE HEADERS INJECTED FOR DOWNLOAD
    cookie_flag = '--cookies cookies.txt' if os.path.exists('cookies.txt') else ''
    ...
    if need_ss:
        await status.edit_text("📤 **Generating 9 Screenshots...**", reply_markup=cancel_btn)
    ...
    try:
        ...
        if need_ss and screenshots:
            try:
                ...
                print(f"Failed to send screenshots album: {e}")
...
# ==========================================
# 🎛️ DYNAMIC KEYBOARDS & BATCH PARSER
# ==========================================
...
async def parse_episodes_from_url(url, status_msg):
    ...
    if "hanime.tv" in url:
        ...
        if match:
            await status_msg.edit_text("🔍 **Smart Auto-Discover ON!** Checking next episodes... 🕵️‍♂️")
        ...
        if len(episodes_to_download) <= 1:
            await status_msg.edit_text("⏳ **Scanning Playlist structure via yt-dlp...** 🕵️‍♂️")
            cookie_flag = '--cookies cookies.txt' if os.path.exists('cookies.txt') else ''
            # 🔥 yt-dlp playlist parsing ke liye bhi YouTube Headers aur Cookies
...
async def start(client, message):
    await message.reply_text(
        ...
        "👉 `/queue <link>` - Add Series/Playlist to background queue (10s gap)\n"
        "👉 `/batch <link>` - Download Series/Playlist immediately\n"
        "👉 `/ss <link>` - Video + Screenshots\n"
        "👉 `/ytdlleech <link>` - YouTube/Insta with Bypass Headers"
    )
...
async def handle_queue(client, message: Message):
    ...
    status = await message.reply_text("⏳ **Analyzing links for Queue (Batch Mode)...** 🕵️‍♂️")

    try:
        url = urls[0]
        # Batch ki tarah saare episodes nikalega
        episodes_to_download = await parse_episodes_from_url(url, status)
...
async def handle_batch(client, message: Message):
    ...
    try:
        ...
        if episodes_to_download:
            ...
            ss_text = "📸 (+ SS)" if need_ss else ""
            await status.edit_text(f"🔍 **Found {len(episodes_to_download)} episodes!** {ss_text}\n\nKaunsi Quality me saare download karne hain?", reply_markup=get_quality_keyboard(task_id, qualities))
        else: await status.edit_text("❌ **No valid episodes found.**")
    except Exception as e: await status.edit_text(f"❌ **Error:**\n`{str(e)}`")
...
async def handle_ytdlleech(client, message: Message):
    ...
    if title:
        ...
        await status.edit_text(f"🎬 **{title[:50]}...**\n\n✅ **yt-dlp Supported!** Quality?", reply_markup=get_quality_keyboard(task_id, qualities))
    else: await status.edit_text("❌ **Extraction Failed.**\nLink might be private.")
...
async def handle_ss_command(client, message: Message):
    ...
    url = message.text.split(maxsplit=1)[1].strip()
    status = await message.reply_text("⏳ **Aggregating Qualities (Video + SS Mode)...** 🕵️‍♂️")
...
async def quality_callback(client, query):
    ...
    if task["type"] == "queue":
        q_data = {
            "original_msg": original_msg, "episodes": task["episodes"], 
            "quality": quality, "task_id": task_id, "need_ss": need_ss
        }
        await task_queue.put(q_data)
        position = task_queue.qsize()
        await query.message.edit_text(f"✅ **Series Added to Queue!** 🚦\n**Position in Queue:** `{position}`\nBot will process all episodes and then take a 10s break automatically.")
        return 
...
async def cancel_callback(client, query):
    ...
    if query.from_user.id != ACTIVE_TASKS[task_id]["user_id"]:
        return await query.answer("❌ Jisne chalaya hai wahi cancel kar sakta hai.", show_alert=True)
...
if __name__ == "__main__":
    print("🤖 Universal Auto-DL Bot with Smart Aggregator & Queue is Alive...")
    keep_alive() 
