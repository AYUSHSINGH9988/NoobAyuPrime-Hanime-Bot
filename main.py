...
# ==========================================
# 🛡️ PREMIUM ROTATING PROXY CONFIGURATION
# ==========================================
...
class HealthCheckHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Bot is ALIVE with Premium Proxy & Smart Scanner!")
...
async def get_video_info_async(url):
    try:
        cmd = f'yt-dlp --proxy "{PROXY_URL}" -j "{url}"'
...
async def process_video(client, original_message, vid_title, video_url, yt_thumb, yt_duration, quality, task_id, need_ss=False):
    ...
    if cached_video:
        ...
        await status.delete()
        # Cached me album nahi bhej rahe taaki spam na ho. Agar chahiye toh yaha DB fetch lagana padega.
        return True

    await status.edit_text(f"📥 **Downloading:** `{vid_title[:30]}...`\n⚙️ **Quality:** `{quality}p`\n🌐 **Source:** `{video_url.split('/')[2]}`", reply_markup=cancel_btn)
    file_name = f"{vid_title}_{quality}p.mp4"
    screenshots = []

    cmd = f'yt-dlp --proxy "{PROXY_URL}" --newline -S "res:{quality}" -o "{file_name}" "{video_url}"'
    ...
    # 🔥 Generate 9 Screenshots ONLY IF requested via /ss or /batch
    if need_ss:
    ...
    try:
        ...
        # Delete generated screenshots from server to save space
...
async def start(client, message):
    await message.reply_text(
        ...
        "👉 Use `/ss <link>` for Video + Screenshots Album.\n"
        "👉 Use `/batch <link>` for Series (Auto Aggregates from all sites!).\n"
...
async def handle_ytdlleech(client, message: Message):
    ...
    if title:
        ...
        # Simulate streams dict for compatibility
...
@app.on_message((filters.text | filters.command("dl")) & ~filters.command(["start", "batch", "ytdlleech", "ss"]))
...
async def handle_batch(client, message: Message):
    ...
    # Optional: allow batch with screenshots if user types "/batch ss http..."
    need_ss = False
        ...
        if len(urls) == 1 and "hanime.tv" in url:
            ...
            if r.status_code == 200:
                ...
                for vid in franchise_videos:
                    episodes_to_download.append({'slug': vid['slug']})
            else:
                return await status.edit_text("❌ **API failed.**")
        else:
            # Multi URL or Smart Auto-Discover logic
            if len(urls) > 1:
                ...
                if len(episodes_to_download) <= 1:
                    episodes_to_download = [{'url': url, 'title': 'Single Episode inside Batch'}]

        if episodes_to_download:
            # Use the first episode to find qualities
            ...
            ss_mode_text = "📸 (+ Screenshots)" if need_ss else ""
            await status.edit_text(f"🔍 **Found {len(episodes_to_download)} episodes!** {ss_mode_text}\n\nKaunsi Quality me saare download karne hain? 👇", reply_markup=get_quality_keyboard(task_id, qualities))
...
async def quality_callback(client, query):
    ...
    if task["type"] == "single":
        # Stream dict se best URL uthayega jo is quality ka ho
        ...
        for index, vid in enumerate(franchise_videos):
            ...
            # Batch loop me har episode ko scan karo taaki best source mile
...
if __name__ == "__main__":
    print("🤖 Universal Auto-DL Bot with Smart Aggregator is Alive...")
    keep_alive() 
    app.run()
