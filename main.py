...
class HealthCheckHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Bot is ALIVE with Secure Group Support!")
...
# 🌟 Helper to get User ID (Supports Anonymous Group Admins)
...
async def start(client, message):
    await message.reply_text(
        ...
        "👉 Use `/ytdlleech <link>` for Generic Sites."
    )

# 🌟 NEW: Handles text links AND /dl command for groups
...
async def handle_message(client, message: Message):
    ...
    if title:
        ...
        # 🔒 SAVE USER ID FOR GROUP SECURITY
...
async def handle_ytdlleech(client, message: Message):
    if len(message.command) < 2: return await message.reply_text("❌ **Oops! URL missing.**\nFormat: `/ytdlleech <link>`")
    ...
    if title:
        task_id = str(uuid.uuid4())[:8]
        PENDING_TASKS[task_id] = {
            "type": "single", "url": vid_url, "title": title, "thumb": thumb, 
            "duration": duration, "original_msg": message, "user_id": get_user_id(message)
        }
        await status.edit_text(f"🎬 **{title[:50]}...**\n\n✅ Found Available Qualities! Select one: 👇", reply_markup=get_quality_keyboard(task_id, qualities))
    else: await status.edit_text("❌ **Extraction Failed.**")
...
if __name__ == "__main__":
    print("🤖 Universal Auto-DL Bot is Alive with Secure Group Features...")
    keep_alive() 
    app.run()