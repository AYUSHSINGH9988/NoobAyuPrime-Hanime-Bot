...
class HealthCheckHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Bot is ALIVE with Dual yt-dlp System!")
...
def get_video_info(url, is_generic=False):
    try:
        ...
        qualities = [1080, 720, 480, 360] # Default fallback
        
        # Sirf Generic sites ke liye detail me formats dhoondhega (Hanime plugin bach jayega)
        if is_generic:
            try:
...
async def process_video(client, original_message, vid_title, video_url, yt_thumb, yt_duration, quality):
    status = await original_message.reply_text(f"🔍 **Checking Database for:** `{vid_title[:30]}...` **({quality}p)**")
...
async def start(client, message):
    welcome_text = (
        ...
        "✅ Dynamic Quality Selector & MongoDB Cache\n\n"
        "**Commands:**\n"
        "👉 Paste `Hanime link` directly.\n"
        "👉 Use `/batch <hanime-link>` for Series.\n"
        "👉 Use `/ytdlleech <link>` for Any other site.\n"
    )
    await message.reply_text(welcome_text)

# 🌟 1. SUPPORTED SITES ONLY (Direct Paste)
@app.on_message(filters.text & ~filters.command(["start", "batch", "ytdlleech"]))
async def handle_message(client, message: Message):
    url = message.text
    if not any(site in url for site in SUPPORTED_SITES): 
        return # Ignore normal text or unsupported links

    status = await message.reply_text("⏳ **Scanning Hanime Link...** 🕵️‍♂️")
    ...
    if title:
        ...
        PENDING_TASKS[task_id] = {
            "type": "single",
            "url": url, # Using original URL for plugin
            "title": title,
            "thumb": thumb,
            "duration": duration,
...
# 🌟 2. NEW COMMAND: /ytdlleech (For Everything Else)
...
async def handle_ytdlleech(client, message: Message):
    ...
    if title:
        ...
        PENDING_TASKS[task_id] = {
            "type": "single",
            "url": vid_url, # Important: Using direct stream URL for generic yt-dlp
            "title": title,
            "thumb": thumb,
            "duration": duration,
            "original_msg": message
...
# 🌟 3. BATCH COMMAND (For Supported Sites)
...
async def quality_callback(client, query):
    ...
    if task["type"] == "single":
        ...
        for index, vid in enumerate(franchise_videos):
            ...
            title, _, thumb, duration, _ = get_video_info(vid_url, is_generic=False)
            if title:
...
if __name__ == "__main__":
    print("🤖 Universal Auto-DL Bot is Alive with Dual yt-dlp System...")
    keep_alive() 
    app.run()
