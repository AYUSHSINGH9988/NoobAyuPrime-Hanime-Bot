...
# 🛠️ NAYA METADATA EXTRACTOR (FFMPEG SE)
async def extract_metadata(video_path):
    ...
    # Extract Thumbnail
    try:
        thumb_cmd = f'ffmpeg -y -i "{video_path}" -ss 00:00:02 -vframes 1 "{thumb_path}"'
...
# ==========================================
# 📥 DOWNLOAD, UPLOAD & MONGODB LOGIC
# ==========================================
async def process_video(client, original_message, vid_title, m3u8_url):
    status = await original_message.reply_text(f"🔍 **Checking Database for:** `{vid_title}`...")
    
    # MONGODB CACHE CHECK
    cached_video = await file_cache.find_one({"title": vid_title})
    if cached_video:
        await status.edit_text(f"⚡ **Video Found in Database! Fast Forwarding...**")
        await client.copy_message(
            chat_id=original_message.chat.id, 
            from_chat_id=DUMP_CHAT_ID, 
            message_id=cached_video["msg_id"]
        )
        await status.delete()
        return
    ...
    # 🛠️ FFMPEG se asli Thumbnail aur Duration nikal rahe hain
    ...
    try:
        if "hanime.tv" in url:
            ...
            if r.status_code == 200:
                ...
                await status.edit_text(f"🔍 **Found {len(franchise_videos)} episodes! Extracting and Checking Cache...**")
...
if __name__ == "__main__":
    print("🤖 Universal Auto-DL Bot is Alive with MongoDB...")
    app.run()
