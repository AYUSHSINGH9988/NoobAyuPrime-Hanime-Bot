...
class HealthCheckHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Bot is ALIVE with Interactive Playlist Selector!")
...
async def process_video(client, original_message, vid_title, video_url, yt_thumb, yt_duration, quality, task_id):
    ...
    if not os.path.exists(file_name):
        await status.edit_text(f"❌ **Download Failed.**\nShayad ye quality available nahi hai.")
        return False
...
async def render_playlist_keyboard(message, task_id):
    ...
    videos = task["episodes"]
    selected = task["selected"]
    buttons = []
    
    # Render individual toggle buttons
    for i, vid in enumerate(videos):
        prefix = "✅" if i in selected else "❌"
        # Truncate title so Telegram doesn't crash (max 64 bytes for callback, UI button limits)
        title = vid['title'][:30] + ".." if len(vid['title']) > 30 else vid['title']
        buttons.append([InlineKeyboardButton(f"{prefix} {title}", callback_data=f"tgl_{task_id}_{i}")])
    ...
    await message.edit_text(
        f"📋 **Custom Playlist Detected!**\n\nFound {len(videos)} videos. Select the ones you want to download:",
...
async def handle_message(client, message: Message):
    ...
    if title:
        ...
        PENDING_TASKS[task_id] = {
            "type": "single", "url": url, "title": title, "thumb": thumb, "duration": duration, "original_msg": message
        }
...
# 🌟 ULTIMATE BATCH COMMAND (With Playlist UI) 🌟
...
async def handle_batch(client, message: Message):
    ...
    try:
        # Scenario 1: Hanime Custom Playlist (The new feature you asked for)
        if len(urls) == 1 and "hanime.tv/playlists/" in urls[0]:
            url = urls[0]
            await status.edit_text("⏳ **Scanning Custom Playlist via yt-dlp...** 🕵️‍♂️")
            ...
            for line in lines:
                ...
                try:
                    ...
                    if ep_url: playlist_videos.append({'url': ep_url, 'title': data.get('title', 'Unknown Episode')})
                except: pass
            
            if playlist_videos:
                # Telegram has a limit on buttons. We limit to top 40 items to prevent crashes.
                if len(playlist_videos) > 40: playlist_videos = playlist_videos[:40] 

                task_id = str(uuid.uuid4())[:8]
                PENDING_TASKS[task_id] = {
                    "type": "playlist_selection", 
                    "site": "generic", 
                    "episodes": playlist_videos, 
                    "selected": list(range(len(playlist_videos))), # Select all by default
                    "original_msg": message
                }
                await render_playlist_keyboard(status, task_id)
            else:
                await status.edit_text("❌ **No videos found in this custom playlist.**")
            return

        # Scenario 2: Hanime Native Series (Franchise)
        elif len(urls) == 1 and "hanime.tv" in urls[0]:
            url = urls[0]
            ...
            if r.status_code == 200:
                ...
                task_id = str(uuid.uuid4())[:8]
                PENDING_TASKS[task_id] = {
                    "type": "batch", "site": "hanime", "episodes": franchise_videos, "original_msg": message
                }
        ...
        # Scenario 3: Generic Site Multi-Links or Auto-Discover
        ...
        if episodes_to_download:
            ...
            task_id = str(uuid.uuid4())[:8]
            PENDING_TASKS[task_id] = {
                "type": "batch", "site": "generic", "episodes": episodes_to_download, "original_msg": message
            }
            await status.edit_text(
                f"🔍 **Found {len(episodes_to_download)} episodes!**\n\nKaunsi Quality me saare download karne hain? 👇",
...
async def toggle_all(client, query):
    ...
    task["selected"] = list(range(len(task["episodes"])))
    await render_playlist_keyboard(query.message, task_id)
...
async def done_selection(client, query):
    ...
    if not selected_indices:
        return await query.answer("⚠️ Bhai, kam se kam 1 video toh select karo!", show_alert=True)
    
    # Filter only selected episodes
    final_episodes = [task["episodes"][i] for i in sorted(selected_indices)]
    task["episodes"] = final_episodes
    task["type"] = "batch" # Switch mode to batch processing
    
    await query.message.edit_text("⏳ **Analyzing selected videos...**")
    
    first_vid_url = final_episodes[0]['url']
    _, _, _, _, qualities = get_video_info(first_vid_url, is_generic=True)
    
    await query.message.edit_text(
        f"🔍 **Ready to download {len(final_episodes)} selected videos!**\n\nKaunsi Quality chahiye? 👇",
        reply_markup=get_quality_keyboard(task_id, qualities)
    )
...
if __name__ == "__main__":
    print("🤖 Universal Auto-DL Bot is Alive with Interactive Playlist UI...")
    keep_alive() 
    app.run()