...
# ==========================================
# Aapki proxy yahan set kar di gayi hai
...
# ==========================================
# 🌐 KOYEB / RENDER / HEROKU HEALTH CHECK FIX 
# ==========================================
class HealthCheckHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Bot is ALIVE with Premium Proxy & Unbreakable Clean UI!")
...
# ==========================================
# 🌟 DUAL EXTRACTOR LOGIC WITH PROXY
# ==========================================
def get_video_info(url, is_generic=False):
    try:
        # Proxy added to yt-dlp info extractor
        command = f'yt-dlp --proxy "{PROXY_URL}" -j "{url}"'
...
# ==========================================
# 📥 DOWNLOAD & UPLOAD LOGIC WITH PROXY
# ==========================================
async def process_video(client, original_message, vid_title, video_url, yt_thumb, yt_duration, quality, task_id):
    ...
    # 🔥 Proxy injected into yt-dlp download command
...
async def handle_message(client, message: Message):
    ...
    if title:
        ...
        await status.edit_text("❌ **Extraction Failed.**\nShayad link expire ho gaya hai ya site proxy block kar rahi hai.")
    ...
    try:
        ...
        if len(urls) > 1:
            ...
            if len(episodes_to_download) <= 1:
                ...
                # 🔥 Proxy injected into yt-dlp playlist extraction
                cmd = f'yt-dlp --proxy "{PROXY_URL}" -j --flat-playlist "{url}"'
...
if __name__ == "__main__":
    print("🤖 Universal Auto-DL Bot is Alive with Premium Proxy...")
    keep_alive() 
    app.run()
