...
class HealthCheckHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Bot is ALIVE with Multi-Link Queue System!")
...
PENDING_TASKS = {} 
ACTIVE_TASKS = {} 
SUPPORTED_SITES = ["hanime.tv", "hstream.moe", "oppai.stream", "hentaihaven.com", "hentaimama.io"]
...
# ==========================================
# 🚦 MASTER QUEUE WORKER (Multi-Link)
# ==========================================
...
async def queue_worker():
    while True:
        ...
        urls = q_data['urls']  # List of series links
        ...
        try:
            for s_idx, series_url in enumerate(urls):
                ...
                await original_msg.reply_text(f"🔍 **Scanning Series {s_idx+1}/{len(urls)}:**\n`{series_url}`")
                
                # Background me episodes nikalna (Silent parsing)
                episodes = await parse_episodes_from_url_silent(series_url)
                
                if not episodes:
                    await original_msg.reply_text(f"❌ **No valid episodes found for:** `{series_url}`")
                    continue
                
                await original_msg.reply_text(f"🚀 **Starting Series {s_idx+1} ({len(episodes)} items)...**")
                
                # Series ke andar ke episodes download karna
                ...
                # ⏱️ 10 SECOND SLEEP BETWEEN SERIES (Links)
                if s_idx < len(urls) - 1:
                    await original_msg.reply_text(f"🎉 **Series {s_idx+1} Complete!**\n⏳ *Taking a 10-second break before starting Series {s_idx+2}...*")
                    await asyncio.sleep(10)
            
            if not ACTIVE_TASKS.get(task_id, {}).get("cancel"):
                await original_msg.reply_text(f"✅ **All {len(urls)} Queued Tasks Completed!**")
...
async def handle_queue(client, message: Message):
    ...
    # Split links by spaces or newlines
    ...
    try:
        # Pata lagane ke liye qualities, sirf pehle link ka ek scan karenge
